...
from data_itzi import get_training_set, get_test_set
import torch.backends.cudnn as cudnn

# Training settings
parser = argparse.ArgumentParser(description='pix2pix-PyTorch-implementation')
parser.add_argument('--dataset', required=True, help='dataset we want to use')
...
def train(epoch):
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        fname_csv = "loss_log.csv"
        with open(fname_csv, "a") as csv_file:
            writer = csv.writer(csv_file, delimiter=',')
            writer.writerow(['{},{}'.format(loss_d.data[0],loss_g.data[0])])
        fname = "loss_log.txt"
        with open(fname, "a") as f:
            f.write("===> Epoch[{}]({}/{}): Loss_D: {:.4f} Loss_G: {:.4f}".format(
...
def checkpoint(epoch):
    ...
    net_g_model_out_path = "checkpoint/{}/netG_transfer_epoch_{}.pth".format(opt.dataset, epoch)
    net_d_model_out_path = "checkpoint/{}/netD_transfer_epoch_{}.pth".format(opt.dataset, epoch)
